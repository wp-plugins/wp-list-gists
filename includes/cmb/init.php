	...
	function show() {
		...
		echo '<div class="meta-box">';
			...
			switch ( $field['type'] ) {
				...
				case 'textarea_code':
					echo '<p class="cmb_metabox_description">', $field['desc'], '</p>';
					echo '<textarea name="', $field['id'], '" id="', $field['id'], '" cols="60" rows="10" class="cmb_textarea_code">', '' !== $meta ? $meta : $field['std'];
					break;					
				case 'select':
				...
				case 'radio_inline':
					...
					$i = 1;
					foreach ($field['options'] as $option) {
						echo '<div class="cmb_radio_inline_option"><label for="', $field['id'], $i, '"><input type="radio" name="', $field['id'], '" id="', $field['id'], $i, '" value="', $option['value'], '"', $meta == $option['value'] ? ' checked="checked"' : '', ' /> ', $option['name'], '</label></div>';
						$i++;
					}
					echo '</div>';
					break;
